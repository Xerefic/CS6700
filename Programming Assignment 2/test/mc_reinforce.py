...
def finish_episode():
    ...
    policy_loss = []
    returns = deque()
    gammas = torch.arange(len(policy.rewards[::-1]))
    for idx, r in enumerate(policy.rewards[::-1]):
        R = r*torch.pow(args.gamma, gammas[idx])
    ...
    optimizer.zero_grad()
    policy_loss = (returns * torch.cat(policy_loss)).sum()
    gradients_wrt_params(policy, policy_loss)
    update_params(policy, 1e-3)
